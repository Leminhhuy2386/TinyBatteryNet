...
def calculate_metrics_based_on_seen_number_of_cycles(total_preds, total_references, total_seen_number_of_cycles, alpha1, alpha2, model, dataset, seed, finetune_dataset, start=1, end=100):
    ...
    for number in range(start, end+1):
        ...
        hit_num = sum(relative_error<=alpha1)
    ...
    try:
        ...
        xs = sorted(number_MAPE.keys())
        ys = [number_MAPE[x] for x in xs]
        ys_acc1 = [number_alphaAcc1[x] for x in xs]
        ys_acc2 = [number_alphaAcc2[x] for x in xs]

        fig, ax1 = plt.subplots(figsize=(8.5, 4.5))
        ax1.plot(xs, ys, color='#2166AC', marker='o', linewidth=1.8, markersize=3.5, label='MAPE')
        ax1.set_xlabel('Seen cycle number')
        ax1.set_ylabel('MAPE', color='#2166AC')
        ax1.tick_params(axis='y', labelcolor='#2166AC')
        ax1.grid(True, alpha=0.25)

        ax2 = ax1.twinx()
        ax2.plot(xs, ys_acc1, color='#D95F02', marker='s', linewidth=1.4, markersize=3, alpha=0.85, label=f'Acc@{int(alpha1*100)}%')
        ax2.plot(xs, ys_acc2, color='#1B9E77', marker='^', linewidth=1.4, markersize=3, alpha=0.85, label=f'Acc@{int(alpha2*100)}%')
        ax2.set_ylabel('Accuracy (%)')

        title_dataset = finetune_dataset if finetune_dataset not in [None, '', 'None'] else dataset
        ax1.set_title(f'MAPE by cycle number: {model} on {dataset} / {title_dataset}')
        handles1, labels1 = ax1.get_legend_handles_labels()
        handles2, labels2 = ax2.get_legend_handles_labels()
        ax2.legend(handles1 + handles2, labels1 + labels2, loc='best', fontsize=8, framealpha=0.9)
        fig.tight_layout()
        fig.savefig(f'{output_path}mape_by_cycle_{model}_{dataset}_{finetune_dataset}_{seed}.png', dpi=300, bbox_inches='tight')
        plt.close(fig)
    except Exception as e:
        print(f'Warning: could not save cycle plot: {e}')
