"""
TinyBatteryNetV1R_NoGate — Ablation: Cycle gate removed (mask-only).
Architecture identical to TinyBatteryNetV1R except the learned sigmoid
cycle gate is replaced by a plain validity mask (0/1 per cycle).
"""
import torch
import torch.nn as nn
from torch.nn.utils.rnn import pack_padded_sequence, pad_packed_sequence
from models.TinyBatteryNetV1 import _MultiScalePyramid, _SEBlock
...
class Model(nn.Module):
    ...
    def __init__(self, configs) -> None:
        ...
        # No cycle_gate linear — gate is disabled at forward time
        self.gru = nn.GRU(
    ...
    def forward(self, cycle_curve_data: torch.Tensor, curve_attn_mask: torch.Tensor,
        ...
        x = self.intra_drop(x)
        x = x.view(B, L, d)
        # No learned gate — apply binary validity mask only
        x = x * curve_attn_mask.unsqueeze(-1)
